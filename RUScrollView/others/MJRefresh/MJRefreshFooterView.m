...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state)
    {
		...
		case MJRefreshStateNormal:
        {
            ...
            if (animDuration == 0 && self.needShowNoticeWhenNoData) {//需要提示最后一行时执行
                _arrowImage.hidden = YES;
                _statusLabel.text = MJRefreshFooterLast;
                _scrollView.userInteractionEnabled = NO; //当需要显示最后一行提示时阻止用户再次上拉操作
                _scrollView.contentOffset = tempOffset;
                [self performSelector:@selector(animationWithDuration:) withObject:@(0.3f) afterDelay:0.25f];
            }
            else {
                [self animationWithDuration:@(0.3f)];
            }
...
- (void)animationWithDuration:(NSNumber *)interval
{
    [UIView animateWithDuration:[interval floatValue] animations:^{
        UIEdgeInsets inset = _scrollView.contentInset;
        inset.bottom = _scrollViewInitInset.bottom;
        _scrollView.contentInset = inset;
    }];
    [self performSelector:@selector(restoreStatus) withObject:nil afterDelay:0.3];
}

- (void)restoreStatus
{
    _scrollView.userInteractionEnabled = YES; //当最后一行提示时消失时允许用户再次上拉操作
    _arrowImage.hidden = NO;
    _arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
    _statusLabel.text = MJRefreshFooterPullToRefresh;
}
